...
def _shard_batch(*arrays, multiple: int = NUM_DEVICES):
    if not arrays:
        raise ValueError('No arrays provided to _shard_batch')
    arrays = [np.asarray(a) for a in arrays]
    base_n = int(arrays[0].shape[0])
    for arr in arrays[1:]:
        if int(arr.shape[0]) != base_n:
            raise ValueError('All arrays passed to _shard_batch must have the same batch dimension')
    sharded = []
    original_n = None
    for arr in arrays:
        padded, original_n = _pad_to_multiple(arr, multiple=multiple)
        per_device = padded.shape[0] // multiple
        sharded.append(padded.reshape(multiple, per_device, *padded.shape[1:]))
    return sharded[0] if len(sharded) == 1 else tuple(sharded), original_n


def _unshard_batch(arr, original_n: int):
    arr = np.asarray(arr)
    if arr.ndim >= 2 and arr.shape[0] == NUM_DEVICES:
...
def _make_jax_train_step(apply_fn, opt_update, get_params):
    if PMAP_ENABLED:
        @partial(jax.pmap, axis_name='batch', in_axes=(None, None, 0, 0, None, None))
...
def _make_jax_eval_step(apply_fn):
    if PMAP_ENABLED:
        @partial(jax.pmap, axis_name='batch', in_axes=(None, 0, 0))
...
def train_fold(fold_idx, fold_info, train_df, model_name, mcfg, arch_dir):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch in train_loader:
            ...
            step += 1
            epoch_losses.append(float(np.asarray(loss).mean()))
            epoch_accs.append(float(np.asarray(acc).mean()))
